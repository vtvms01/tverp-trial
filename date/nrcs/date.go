// Package util cung cấp các tiện ích ngày dùng chung.
// Các helper ngày — múi giờ chuẩn Asia/Ho_Chi_Minh.
package util
...
// DateRangeInVN parse chuỗi from/to dạng YYYY-MM-DD thành khoảng nửa mở theo múi giờ VN.
// Khoảng ngày nửa mở [from, to_exclusive) cho truy vấn theo khoảng:
//
//   - from_date: đầu ngày ở Asia/Ho_Chi_Minh — bao gồm (00:00:00+07:00)
//   - to_date:   đầu ngày KẾ TIẾP ở Asia/Ho_Chi_Minh — loại trừ (ngày-sau 00:00:00+07:00)
//
// Bên gọi truy vấn PHẢI dùng khoảng timestamp `WHERE event_at >= fromAt AND event_at < toAtExclusive`
// để giữ index PG trên TIMESTAMPTZ còn dùng được (tránh bọc DATE(col)).
//
// Trả về (zero, zero, err) khi parse lỗi hoặc khoảng bị đảo (fromAt >= toAtExclusive)
// — chốt chặn F-R1-6 tránh kết quả rỗng âm thầm khi người dùng vô tình hoán đổi tham số.
